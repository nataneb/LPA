...
DEFINE_int32(labels_total, 7, "Total number of different labels (gender,age,etc.)");
DEFINE_int32(label_number, 1, "Number of label (in seed file) used in label propagation");
...
struct LabelPropagation : public GraphlabVertexProgram<G, label_counter> {
  ...
  /* Getting the most probable label for a node and making a decision to activate() a node for next iteration
       activate() for a node means that it does not meet the criterion we use for stopping, namely the number of iterations for Iteration-Clamping strategy*/
  ...
  run([]{
    ...
    for (int i = 0; i < FLAGS_trials; i++) {
      ...
      GRAPPA_TIME_REGION(total_time) {
        ...
        double current=walltime();

        //starting propagation
        NaiveGraphlabEngine<G,LabelPropagation>::run_sync(g);
        double total=walltime()-current;
        ofs<<FLAGS_path<<" "<<total<<std::endl;
        ofs.close();
